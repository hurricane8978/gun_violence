...
First, let's see the general trend and seasonality of our data between 2018-2021 and generate the time series. 
...
Then, we  did a box-cox transformation to make the variance equal. Because the lambda we got is `r BoxCox.lambda(month.ts)`, we decided to square root our series according to the common powers in box-cox transformation. 
...
Meanwhile, we also performed Ljung-Box test to check if the pattern we obtained from the series is because of randomness. The p-values are all less than 0.05, and we conclude that the correlation is not due to randomness.
